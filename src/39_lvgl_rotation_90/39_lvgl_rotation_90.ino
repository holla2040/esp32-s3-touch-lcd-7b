/*
 * 39_lvgl_rotation_90 - Screen rotation demonstration (90° - Portrait)
 *
 * This example demonstrates LVGL screen rotation at 90 degrees (portrait orientation).
 * The display shows visual indicators to confirm the current orientation.
 *
 * Hardware:
 * - Display: 1024x600 RGB LCD (rotated to portrait, 90° rotation)
 * - Touch: GT911 capacitive touch controller (I2C)
 *
 * Rotation Angle: 90° (Portrait)
 * Physical Orientation: USB port on bottom when viewing screen
 * Logical Resolution: 600x1024 (width x height)
 *
...
void setup() {
    Serial.begin(115200);
    delay(100);
    Serial.println("Starting LVGL rotation demo (90°)...");
    ...
    if (lvgl_port_lock(0)) {
        ...
        lv_label_set_text(title, "Screen Rotation: 90 deg");
        ...
        lv_label_set_text(subtitle, "Portrait | USB Port: BOTTOM");
    ...
    Serial.println("LVGL rotation demo (90°) ready!");
