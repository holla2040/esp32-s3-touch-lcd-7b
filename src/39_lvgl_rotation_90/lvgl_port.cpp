...
static bool lvgl_task_running = false;                   // Flag to control task execution
...
{
    return get_next_frame_buffer((esp_lcd_panel_handle_t)panel_handle); // Return the next frame buffer
}
...
static void flush_dirty_copy(void *dst, void *src, lv_port_dirty_area_t *dirty_area)
{
    ...
    for (int i = 0; i < dirty_area->inv_p; i++) {
        ...
        if (dirty_area->inv_area_joined[i] == 0) {
            ...
            rotate_copy_pixel((const uint16_t *)src, (uint16_t *)dst, x_start, y_start, x_end, y_end, LV_HOR_RES, LV_VER_RES, EXAMPLE_LVGL_PORT_ROTATION_DEGREE);
        }
    }
}
...
static void lvgl_port_task(void *arg)
{
    ...
    // This prevents deadlock during initialization
