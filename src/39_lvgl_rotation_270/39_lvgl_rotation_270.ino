/*
 * 39_lvgl_rotation_270 - Screen rotation demonstration (270° - Inverted Portrait)
 *
 * This example demonstrates LVGL screen rotation at 270 degrees (inverted portrait).
 * The display shows visual indicators to confirm the current orientation.
 *
 * Hardware:
 * - Display: 1024x600 RGB LCD (rotated to inverted portrait, 270° rotation)
 * - Touch: GT911 capacitive touch controller (I2C)
 *
 * Rotation Angle: 270° (Inverted Portrait)
 * Physical Orientation: USB port on top when viewing screen
...
void setup() {
    Serial.begin(115200);
    delay(100);
    Serial.println("Starting LVGL rotation demo (270°)...");
    ...
    if (lvgl_port_lock(0)) {
        ...
        lv_label_set_text(title, "Screen Rotation: 270 deg");
        ...
        lv_label_set_text(subtitle, "Inverted Portrait | USB Port: TOP");
    ...
    Serial.println("LVGL rotation demo (270°) ready!");
