/*
 * 39_lvgl_rotation_0 - Screen rotation demonstration (0° - Default landscape)
 *
 * This example demonstrates LVGL screen rotation at 0 degrees (default orientation).
 * The display shows visual indicators to confirm the current orientation.
 *
 * Hardware:
 * - Display: 1024x600 RGB LCD (landscape, 0° rotation)
 * - Touch: GT911 capacitive touch controller (I2C)
 *
 * Rotation Angle: 0° (Default landscape)
 * Physical Orientation: USB port on right side when viewing screen
 * Logical Resolution: 1024x600 (width x height)
 *
 * To test other rotations, see companion examples:
 * - 39_lvgl_rotation_90: Portrait (600x1024)
...
void setup() {
    Serial.begin(115200);
    delay(100);
    Serial.println("Starting LVGL rotation demo (0°)...");
    ...
    if (lvgl_port_lock(0)) {
        ...
        lv_label_set_text(title, "Screen Rotation: 0 deg");
        ...
        lv_label_set_text(subtitle, "Default Landscape | USB Port: RIGHT");
    ...
    Serial.println("LVGL rotation demo (0°) ready!");
